  ...
  <modal-view v-if="isModalOpen" @on:close="handleModal(false)">
    ...
    <template v-slot:body>
      <div class="h-full p-3"> 
        <button class="mb-3 rounded bg-primary text-white p-2">Agregar Filtro</button> 
        <form>
          <div v-for="optFilter in arrayFilters" :key="optFilter.idFilter.value" class="flex justify-around my-3 w-full">  
            <select class="rounded border-2 border-gray-100 p-1" v-model="optFilter.optionsSearch.value">
              <option v-for="title in allKeys" :key="title" :value="title">{{languagei18n(`nameFields.${title.toUpperCase()}`)}}</option>
            </select>
            <select class="rounded border-2 border-gray-100 p-1" v-model="optFilter.paramSearch.value">
              <option v-for="opt in filterOptions" :key="opt" :value="opt">{{languagei18n(`filterOptions.${opt.toUpperCase()}`)}}</option>
            </select>
            <input type="text" class="rounded border-2 border-gray-100 p-1" v-model="optFilter.searchValueOption.value">
            <button type="">Eliminar Filtro</button>
          </div>
        </form>
      </div>
    </template>
...
export default {
  ...
  setup(props, {emit}) {
    ...
    const arrayFilters = [
      {
        idFilter:ref(0),
        paramSearch: ref(filterOptions[0]),
        optionsSearch: ref(props.allKeys[0]),
        searchValueOption: ref(null)
      }
    ]
    ...
    const handleFilter = () => {
      const {paramSearch,optionsSearch,searchValueOption} = arrayFilters[0]
      filterDataByField(paramSearch.value,optionsSearch.value,searchValueOption.value)
      handleModal(false)
    }
